...
# 创建滑动窗口数据，输入为过去96小时，输出为未来96小时
def create_dataset(data, time_step=96, forecast_horizon=96):
    X, y = [], []
    for i in range(len(data) - time_step - forecast_horizon + 1):
        X.append(data[i:(i + time_step), 0])  # 过去96小时的数据
        y.append(data[(i + time_step):(i + time_step + forecast_horizon), 0])  # 未来96小时的数据
    return np.array(X), np.array(y)


# 创建训练数据
X_train, y_train = create_dataset(scaled_train_values, time_step=96, forecast_horizon=96)
X_test, y_test = create_dataset(scaled_test_values, time_step=96, forecast_horizon=96)
...
for i in range(n_experiments):
    ...
    # 卷积层
    conv_layer = Conv1D(filters=32, kernel_size=7, activation='relu')(input_layer)
    # TCN 编码器
    tcn_output = TCN(nb_filters=32,  # 卷积核数量
                     kernel_size=3,  # 卷积核大小
    ...
    # LSTM 层（如果需要）
    lstm_layer_1 = LSTM(32, return_sequences=True)(tcn_output)
    # 第二个 LSTM 层
    lstm_layer_2 = LSTM(32, return_sequences=False)(lstm_layer_1)
    # 输出层（调整维度）
    output_layer = Dense(96)(lstm_layer_2)  # 输出96个时间步，保持与目标形状一致
    # 构建模型
    ...
    x_input = np.arange(0, 96)  # 输入数据的x轴
    x_pred = np.arange(96, 192)  # 预测数据的x轴
